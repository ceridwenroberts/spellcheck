...
export const SpellsProvider = ({children}) => {
    ...
    useEffect((): void => {
        if(spells.length > 0) {
            const withIncantation: Spell[] = [...spells].filter((spell: Spell): boolean => !!spell.incantation); //double negation makes sure return of boolean, and not string
            const groupedSpells: Record<string, Spell[]> = withIncantation.reduce((acc: Record<string, Spell[]> , spell: Spell): Record<string, Spell[]> => {
